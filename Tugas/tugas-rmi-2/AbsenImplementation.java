...
public class AbsenImplementation extends UnicastRemoteObject implements AbsenInterface {
  ...
  @Override
  public String addStudents(String studentName, String studentId) throws RemoteException {
    ...
    Students student = new Students(nextId++, studentName.trim(), studentId.trim());
    students.add(student);
    return "Mahasiswa berhasil di tambahkan (ID: " + student.id + " - NIM : " + student.studentIds + ")";
  }
  ...
  public String showStudents() throws RemoteException {
    ...
    for (Students s : students) {
      sb.append(s.id).append(". ").append(s.name).append(" - ").append(s.studentIds);
      sb.append("\n");
    }
    return sb.toString();
  }
  ...
  public String searchStudent(String keyword) throws RemoteException {
    ...
    for (Students s : students) {
      if (s.name.toLowerCase().contains(keyword.toLowerCase())
        ...
        sb.append(s.id).append(". ").append(s.studentIds).append(" - ").append(s.name);
        sb.append("\n");
        found = true;
      }
    }
